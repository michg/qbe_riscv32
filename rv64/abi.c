...
sttmps(Ref tmp[], int ntmp, Class *c, Ref mem, Fn *fn)
{
	...
	for (i=0; i<ntmp; i++) {
		...
		emit(Oadd, Kw, r, mem, getcon(c->off[i], fn));
	}
}
...
selcall(Fn *fn, Ins *i0, Ins *i1, Insl **ilp)
{
	...
	for (i=i0, c=ca; i<i1; i++, c++) {
		...
		if (i->op == Oargc) {
			if (c->class & Cstk1) {
				blit(r, off, i->arg[1], 0, 4, fn);
				off += 4;
			}
			if (c->class & Cstk2) {
				blit(r, off, i->arg[1], 4, 4, fn);
				off += 4;
			}
		}
	}
...
selpar(Fn *fn, Ins *i0, Ins *i1)
{
	...
	if (fn->retty >= 0) {
		...
		if (cr.class & Cptr) {
			...
			emit(Ocopy, Kw, fn->retr, TMP(A0), R);
		}
	}
